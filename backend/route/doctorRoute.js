...
doctorRoute.post("/login", async (req, res) => {
    ...
    try {
        bcrypt.compare(password, data.password, function (err, result) {
            if (result) {
                var token = jwt.sign({ doctorId: data._id }, process.env.JWT_SECRET, { expiresIn: "7d" });

                var refreshtoken = jwt.sign({ doctorID: data._id }, process.env.JWT_SECRET, { expiresIn: "7d" });
                res.status(201).send({
